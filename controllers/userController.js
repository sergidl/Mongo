	...
	try {
		...
		if (!body.username || !body.password) {
			...
			if (result != undefined) {
				...
				nosensitive.message = messageusr.user_msg_create

				res.status(201).json(nosensitive);
...
const login = (req, res, next) => {
	...
	try {
		...
		if (!body.username || !body.password || usr != 1 || usr.active===undefined) {
			next(HttpError(400, { message: messageapp.parameter_not_especified }))
		} else {
...
const loginUrl = (req, res, next) => {
	...
	try {
		...
		if (!body || usr != 1|| usr.active===undefined) {
			next(HttpError(400, { message: messageapp.parameter_not_especified }))
		} else {
...
const grants = (req, res, next) => {
	...
	try {
		...
		if (!body.username || usr != 1|| usr.active===undefined) {
			next(HttpError(400, { message: messageapp.parameter_not_especified }))
		} else {
			...
			if (result === undefined) {
				...
				delete nopass.active
								nosensitive.message = messageusr.user_msg_insertgrants
	...
	try {
		...
		if (!body.username || usr != 1|| usr.active===undefined) {
			...
			if (result === undefined) {
				...
				delete nopass.password
				delete nopass.active
				nosensitive.message = messageusr.user_msg_deletegrants
	...
	try {
		...
		if (!body.username || usr != 1|| usr.active===undefined) {
			...
			if (result === undefined) {
				...
				delete nopass.password
				delete nopass.active
				nosensitive.message = messageusr.user_msg_addgrants
		...
		if (!body.username || !body.password || usr != 1|| usr.active===undefined) {
			...
			if (result === undefined) {
				...
				if (!bcrypt.compareSync(body.password, oldpass))
					...
					nosensitive.message = messageusr.user_msg_newpassw
	...
	try {
		...
		if (!body.username || usr != 1) {
			...
			if (result === undefined) {
				...
				delete nopass.password
				delete nopass.active
								nosensitive.message = messageusr.user_msg_deactivateuser
	...
	try {
		...
		if (!body.username || usr != 0) {
			...
			if (result === undefined) {
				...
				delete nopass.password
				delete nopass.active
				nosensitive.message = messageusr.user_msg_activateuser
		...
		if (!body.username || !body.password || usr != 1) {
			...
			if (result === undefined) {
				...
				if (!bcrypt.compareSync(body.password, result.password))
					...
					nosensitive.message = messageusr.user_msg_getusr
